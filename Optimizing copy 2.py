    ...
    def localization(audiowav, ref):
        ...
        for i in range(5):
            for j in range(i+1, 5):
                ...
                cropped_i = localization.crop(audio_channel_i)
                cropped_j = localization.crop(audio_channel_j)
            
                # calculate the mean of the list with peaks using the function process_channel() for each channel
                mean_peak_i = localization.process_channel(cropped_i, ref)
                mean_peak_j = localization.process_channel(cropped_j, ref)
