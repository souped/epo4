...
class KITT:
    ...
    def restart_connection(self):
        ...
        self.serial.close()
        time.sleep(1)
        self.__init__(self.serial.port,baudrate=115200)
    ...
    def emergency_brake(self,from_speed):  # STILL NEEDS TUNING!
        if self.prev_speed > 153 and from_speed==1:
            ...
            self.set_speed(150 - max_speed)  # Set speed to move backwards
...
async def collision_plotter(kitt):  # For the plotter to work with slowly driving towards the wall / stopping
    ...
    while True:
        ...
        elif (kitt.prev_speed >= 150 or kitt.prev_speed < 150) and (kitt.distances[-1][0] < 300 or
                                                                    kitt.distances[-1][1] < 300):
        ...
        elif kitt.prev_speed==150 and (kitt.distances[-1][0] < 300 or kitt.distances[-1][1] < 300):
            # np.savetxt('distance_data.csv', kitt.data, delimiter=',')
            # kitt.data.clear()
